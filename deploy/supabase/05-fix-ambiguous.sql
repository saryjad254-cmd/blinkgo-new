-- ════════════════════════════════════════════════════════════════
-- Fix Bug: "column reference 'order_number' is ambiguous"
--       + previous "record 'v_coupon' is not assigned yet"
--       + previous "function name is not unique"
--
-- 📍 Supabase Dashboard → SQL Editor → New query → الصق كله
-- ════════════════════════════════════════════════════════════════

-- 🗑️ احذف كل النسخ (بكل signature)
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'المطعم غير موجود أو غير نشط'; END IF;
  IF jsonb_array_length(p_items) = 0 THEN RAISE EXCEPTION 'لا توجد عناصر في الطلب'; END IF;
  ...
  -- ✅ FIX: استخدم table-qualified في RETURNING لتجنب ambiguous
