-- Elevated roles must only come from server-managed app_metadata. A user can
-- edit user_metadata during sign-up, so it must never be an authorization source.
...
DECLARE
  ...
  v_display_name text;
BEGIN
  v_requested_role := NEW.raw_app_meta_data->>'role';
