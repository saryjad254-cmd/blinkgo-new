...
export class AuthService {
  ...
  static setSessionCookies(tokens: AuthTokens): void {
    ...
    // v86 fix: cookie format must match @supabase/ssr v0.5.x exactly.
    //
    // @supabase/ssr v0.5.2 (the version pinned in package.json) writes
    // the session cookie as `base64-` + base64url(JSON.stringify(session)).
    // The decode path in @supabase/ssr's storage adapter
    // (`dist/main/cookies.js:142`) only strips the `base64-` prefix and
    // base64url-decodes the rest — if the prefix is missing, the value
    // is returned AS-IS and then fed to `JSON.parse` inside
    // @supabase/auth-js's `getItemAsync`. In theory that still works
    // for a plain JSON session, but in practice (Vercel function
    // runtime + chunked cookies + auto-refresh) the library round-trips
    // the cookie through its OWN setItem path on the very next
    // request, which prepends `base64-`. From that point on the cookie
    // value is `base64-<base64url(json)>` on disk and our setSession
    // would write a plain JSON value ON TOP of it, corrupting the
    // session — the very next call to `supabase.auth.getUser()` from
    // the middleware would either fail JSON.parse or hand a malformed
    // session to auth-js, causing `getUser()` to return `{ user: null }`
    // and the page to be redirected back to /login with
    // `?error=require_role_auth`.
    //
    // To make our writes byte-for-byte identical to what @supabase/ssr
    // writes, encode the session as `base64-<base64url(json)>` here.
    // This is safe for two reasons:
    //   1. Our `cookies.get` adapter (`lib/supabase/server.ts`,
    //      `lib/supabase/middleware.ts`) returns the raw cookie value
    //      as a string and lets @supabase/ssr's storage layer decode
    //      it. The decode path understands BOTH formats (with or
    //      without `base64-` prefix), so a future code path that
    //      reads the cookie with the older raw format will still work.
    //   2. The `base64-` prefix + base64url encoding is what
    //      @supabase/ssr ITSELF writes when it refreshes the session
    //      on the next request, so the cookie stays in a single
    //      canonical format across the login → middleware → page flow.
