...
export function validateProductionRedirectTo(redirectTo: string): string | null {
  ...
  try {
    ...
    // v85: explicit parentheses to make precedence obvious. We reject the
    // apex domain (blinko.de) outright, and we reject www.blinko.de ONLY
    // if it isn't in the production allowlist (it should be — the allowlist
    // is the source of truth). Any other host is checked below by
    // isValidProductionHost().
    if (u.host === APEX_DOMAIN) {
    ...
    if (u.host === `www.${APEX_DOMAIN}` && !ALLOWED_PRODUCTION_HOSTS.has(u.host)) {
      return `Host ${u.host} is the www-form of the apex but not in the production allowlist. ` +
        `Add it to ALLOWED_PRODUCTION_HOSTS or use the canonical host.`;
    }
