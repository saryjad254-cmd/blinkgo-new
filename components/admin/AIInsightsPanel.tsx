...
export function AIInsightsPanel() {
  ...
  const fetch = useCallback(async () => {
    try {
      const res = await apiGet<InsightsResponse>('/api/intelligence/insights', { cacheTtl: 0 });
      if (res.ok && res.data) {
  ...
  useEffect(() => {
    let cancelled = false;
    queueMicrotask(() => { if (!cancelled) void fetch(); });
    const interval = setInterval(fetch, 60_000);
    return () => { cancelled = true; clearInterval(interval); };
  }, [fetch]);

  const severityConfig = {
    critical: { color: 'bg-danger-500/15 text-danger-700 border-danger-500/40', iconColor: 'text-danger-600' },
    warning: { color: 'bg-warning-500/15 text-warning-700 border-warning-500/30', iconColor: 'text-warning-600' },
    info: { color: 'bg-info-500/15 text-info-700 border-info-500/30', iconColor: 'text-info-600' },
  };
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {insight.action?.href && (
                      <Link href={insight.action.href} className="mt-2 inline-flex text-xs font-semibold text-brand-primary hover:underline">
