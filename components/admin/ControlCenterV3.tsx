              ...
              {loading ? (
                ...
                <tr key={u.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 flex gap-1">
                    <button onClick={() => setEditing(u)} aria-label="Edit" className="p-1 text-blue-600 hover:bg-blue-50 rounded focus:outline-none focus:ring-2 focus:ring-blue-500/50"><Edit className="w-3.5 h-3.5" /></button>
                    {!!u.is_active && !!u.is_active && (
                      <button onClick={() => suspend(u.id)} aria-label="Suspend" className="p-1 text-brand-yellow-600 hover:bg-yellow-50 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500/50"><Ban className="w-3.5 h-3.5" /></button>
                    )}
                    {!u.is_active && (
                      <button onClick={() => unsuspend(u.id)} aria-label="Unsuspend" className="p-1 text-green-600 hover:bg-green-50 rounded focus:outline-none focus:ring-2 focus:ring-green-500/50"><Check className="w-3.5 h-3.5" /></button>
                    )}
                  </td>
                </tr>
              ))}
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">
                    {r.is_featured && <span className="px-2 py-0.5 bg-yellow-100 text-brand-yellow-700 text-xs rounded mr-1">Featured</span>}
  ...
  return (
    <>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((c) => (
          <button key={c.id} onClick={() => onNavigate(c.id)} className="bg-white border border-gray-200 rounded-xl p-5 text-left hover:shadow-md transition">
