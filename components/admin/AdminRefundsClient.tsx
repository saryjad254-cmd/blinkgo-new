            ...
            {refunds.length === 0 ? (
                ...
                <td colSpan={7} className="p-8 text-center text-zinc-500">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-zinc-500">
                      {locale === 'ar' ? 'ستظهر هنا طلبات الاسترداد من العملاء.' : locale === 'en' ? 'Customer refund requests will appear here.' : 'Rückerstattungsanfragen von Kunden erscheinen hier.'}
                    </p>
                  </div>
                </td>
              </tr>
              ...
              refunds.map((r) => (
                <tr key={r.id} className="border-b border-zinc-100 dark:border-zinc-800/50">
                  ...
                  <td className="p-4 font-bold">{formatEUR(Number(r.amount))}</td>
                  ...
                  <td className="p-4">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${
                      ...
                      {r.status === 'pending' && <Clock className="mr-1 inline h-3 w-3" />}
                      {r.status === 'completed' && <Check className="mr-1 inline h-3 w-3" />}
                      {r.status}
                    </span>
                  </td>
                  <td className="p-4 text-xs text-zinc-500">{new Date(r.created_at).toLocaleDateString()}</td>
                  <td className="p-4">
                    {r.status === 'pending' && (
                      <button
