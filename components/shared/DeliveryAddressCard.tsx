...
export function DeliveryAddressCard({
  ...
  /**
   * Canonical delivery-zone check (Wesseling + 15 km, see
   * `@/lib/delivery-zone`). Returned to the caller via the address
   * card so the UI can show an at-a-glance "in zone" / "out of zone"
   * badge without duplicating the radius math.
   */
  const zoneCheck = useMemo(() => {
    if (lat == null || lng == null) return null;
    return checkDeliveryZone(lat, lng, addr.postal ?? null);
  }, [lat, lng, addr.postal]);
  ...
  return (
    ...
    >
      ...
      {/* Delivery-zone badge (canonical Wesseling + 15 km) */}
      {zoneCheck && (
        <div
          className={cn(
            'mt-2 inline-flex items-center gap-1.5 px-2 h-6 rounded-md text-[10px] font-bold uppercase tracking-wider',
            zoneCheck.ok
