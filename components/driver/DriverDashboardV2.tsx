...
export function DriverDashboardV2({
  ...
  const handleToggleOnline = useCallback(async () => {
    ...
    try {
      ...
      // v82: warn the driver (and only allow) if they try to go offline
      // while a delivery is still in flight. The server now refuses the
      // transition in that case and returns in_flight_order_id, but the
      // client UX is the right place to ask for confirmation.
      if (!newState && activeOrder) {
        const ok = window.confirm(
          locale === 'ar'
            ? 'لا تزال لديك طلبية قيد التسليم. هل تريد بالتأكيد الاتصال دون اتصال؟ سيُعاد تعيين الطلبية لسائق آخر.'
            : locale === 'en'
            ? 'You still have an order in delivery. Are you sure you want to go offline? The order will be reassigned to another driver.'
            : 'Du hast noch eine Bestellung in Zustellung. Bist du sicher, dass du offline gehen möchtest? Die Bestellung wird einem anderen Fahrer zugewiesen.',
        );
        if (!ok) {
          setBusy(false);
          return;
      ...
      if (res.ok) {
        ...
        if (newState) {
          ...
          setOnlineSince(null);
          // If the server kept the order assigned to us (in-flight), the
          // dashboard should NOT clear the active card — admin still owns
          // the reassignment.
          if (data?.data?.in_flight_order_id) {
            // keep activeOrder as-is; admin can re-dispatch.
          }
        }
      }
    } catch {
