...
export function OrderActions({ orderId, currentStatus }: Props) {
  ...
  // v82 fix: ref-based in-flight guard. `submitting` state is async
  // (React batches re-renders), so a fast double-tap can pass the
  // `if (submitting) return` check twice. The ref is set synchronously.
    ...
    startTransition(async () => {
      try {
        ...
        if (res.ok) {
          ...
          const j = await res.json().catch(() => ({}));
          toast({ type: 'error', message: j?.error?.message || j?.error || (locale === 'ar' ? 'فشل' : locale === 'en' ? 'Failed' : 'Fehler') });
        }
      } catch {} finally {
        inFlightRef.current = false;
      }
