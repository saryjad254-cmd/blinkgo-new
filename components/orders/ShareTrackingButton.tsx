      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 z-50 mt-2 w-80 rounded-2xl border border-zinc-200 bg-white p-4 shadow-2xl dark:border-zinc-800 dark:bg-zinc-900"
          >
            <div className="mb-3 flex items-center justify-between">
              <h4 className="font-bold text-ink-1 dark:text-zinc-100">{t.share.title}</h4>
              <button onClick={() => setOpen(false)} aria-label="Close" className="rounded-full p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-zinc-500/50">
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              <>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    value={shareUrl}
                    readOnly
                    className="flex-1 truncate rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-800"
                  />
                  <button
                    onClick={copyLink}
