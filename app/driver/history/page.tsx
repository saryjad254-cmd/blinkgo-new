    ...
    <div className="min-h-screen bg-bg pb-24" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="relative">
          <label htmlFor="driver-history-search" className="sr-only">
            {locale === 'ar' ? 'ابحث برقم الطلب أو المطعم' : locale === 'en' ? 'Search order # or restaurant' : 'Bestellnr. oder Restaurant suchen'}
          </label>
          ...
          <input
            ...
            className="w-full h-12 ps-10 pe-10 rounded-pill bg-ink-700 border border-edge text-white placeholder:text-text-muted text-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-500/50 focus:outline-none"
          />
          {search && (
            <button
              type="button"
              onClick={() => setSearch('')}
              aria-label={locale === 'ar' ? 'مسح البحث' : locale === 'en' ? 'Clear search' : 'Suche löschen'}
        ...
        {/* Error banner */}
        {error && !loading && (
          <div className="rounded-2xl border border-red-500/40 bg-red-500/10 p-4 flex flex-col items-start gap-3">
                ...
                >
                  ...
                  {expanded && (
                    <div className="px-4 pb-4 pt-2 border-t border-edge space-y-2 text-xs text-text-secondary">
                      ...
                      <div className="flex justify-between">
                        <span>{locale === 'ar' ? 'البقشيش' : 'Tip'}</span>
                        <span className="font-bold text-emerald-400">{formatEUR(Number(it.tip ?? 0))}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>{locale === 'ar' ? 'التاريخ' : 'Date'}</span>
                        <span className="font-bold text-white">{new Date(ts).toLocaleString(locale === 'ar' ? 'ar' : 'de-DE')}</span>
                      </div>
                      <Link
