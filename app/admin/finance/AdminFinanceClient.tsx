  ...
  return (
    <div className="space-y-3">
          ...
          return (
            ...
            >
              <div
                ...
                title={`${s.date}: ${s.revenue.toFixed(2)} € (${s.orders} ${s.orders === 1 ? 'order' : 'orders'})`}
              />
      ...
      <div className="flex justify-between text-[9px] text-text-muted" dir="ltr">
        <span>{new Date(series[0]?.date).toLocaleDateString(locale === 'ar' ? 'ar' : 'de-DE', { day: '2-digit', month: 'short' })}</span>
        <span>{new Date(series[Math.floor(series.length / 2)]?.date).toLocaleDateString(locale === 'ar' ? 'ar' : 'de-DE', { day: '2-digit', month: 'short' })}</span>
        <span>{new Date(series[series.length - 1]?.date).toLocaleDateString(locale === 'ar' ? 'ar' : 'de-DE', { day: '2-digit', month: 'short' })}</span>
      </div>
    </div>
  );
}
