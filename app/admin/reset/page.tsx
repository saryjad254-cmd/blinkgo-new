  ...
  return (
    <div className="min-h-screen bg-bg p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Result banner */}
        {result && (
          <div className={`card glass-card p-4 border-l-4 ${result.ok ? 'border-l-success' : 'border-l-danger'}`}>
            <div className="flex items-center gap-3">
              {result.ok ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-text-muted border-b border-edge-light">
                    ...
                    <th className="text-right py-2 px-2 font-semibold">{(locale === 'ar' ? 'الطلبات' : locale === 'en' ? 'Orders' : 'Bestellungen')}</th>
                    <th className="text-right py-2 px-2 font-semibold">{(locale === 'ar' ? 'موصّل' : locale === 'en' ? 'Delivered' : 'Geliefert')}</th>
                    <th className="text-right py-2 px-2 font-semibold">Storniert</th>
                    <th className="text-right py-2 px-2 font-semibold">{(locale === 'ar' ? 'الإيرادات' : locale === 'en' ? 'Revenue' : 'Umsatz')}</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map((h) => (
                    <tr key={h.date} className="border-b border-edge-light/50 hover:bg-surface-elevated/50">
                      ...
                      <td className="py-2 px-2 text-right text-white">{h.total_orders}</td>
                      <td className="py-2 px-2 text-right text-success">{h.delivered_orders}</td>
                      <td className="py-2 px-2 text-right text-danger">{h.cancelled_orders}</td>
                      <td className="py-2 px-2 text-right text-brand-red-500 font-bold">{formatEUR(Number(h.total_revenue))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="space-y-3 mb-4">
              <div className="bg-surface-elevated rounded-md p-3 text-sm">
                ...
                <input
                  ...
                  aria-label='RESET TODAY Bestätigung'
                  className="w-full px-3 py-2 bg-bg border border-edge rounded-md text-white placeholder:text-text-muted focus:outline-none focus:border-brand-red-500 focus:ring-2 focus:ring-brand-red-500/50"
                  autoFocus
                />
              </div>
            </div>
