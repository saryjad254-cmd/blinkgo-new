          ...
          <div>
            ...
            {lastUpdate && (
              <p className="text-[10px] text-text-muted mt-0.5">
                {t.lastUpdate}: {lastUpdate.toLocaleTimeString(locale === 'ar' ? 'ar' : 'de-DE')}
              </p>
            )}
          </div>
          <button
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <ListCard
            ...
            items={drivers.map((d) => ({
              ...
              extra: d.last_location_at ? new Date(d.last_location_at).toLocaleTimeString(locale === 'ar' ? 'ar' : 'de-DE', { hour: '2-digit', minute: '2-digit' }) : '—',
            }))}
