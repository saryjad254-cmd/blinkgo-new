...
export function AdminOrdersClient({
  ...
  // v82: surface a recoverable error state when the orders endpoint fails
  // (network 5xx, RLS denial, table missing). Previously silent.
    ...
    <AdminLayout user={user} locale={locale}>
      <div className="space-y-5" dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <input
          ...
          aria-label={t.search}
          className="w-full h-10 px-4 rounded-xl bg-ink-700 border border-edge text-text placeholder:text-text-muted focus:border-brand-500 focus:ring-2 focus:ring-brand-500/40 focus:outline-none"
          dir="ltr"
        />
        ...
        <div className="rounded-2xl bg-surface-elevated border border-edge overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center space-y-2">
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-red-500/15 text-red-400">⚠</div>
              <p className="text-sm text-text font-bold">{error}</p>
              <p className="text-xs text-text-muted">
                {isAr ? 'يرجى التحقق من الاتصال والمحاولة مجدداً' : locale === 'en' ? 'Please check your connection and retry.' : 'Bitte prüfe die Verbindung und versuche es erneut.'}
              </p>
              <button
                type="button"
                onClick={fetchOrders}
                className="mt-3 inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-ink-700 border border-edge text-sm font-bold text-text-secondary hover:text-white"
              >
                {isAr ? 'إعادة المحاولة' : locale === 'en' ? 'Retry' : 'Erneut versuchen'}
              </button>
            </div>
          ) : orders.length === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              <div className="w-12 h-12 rounded-2xl bg-ink-700/50 flex items-center justify-center mb-3">
                <ShoppingBag className="w-6 h-6 text-text-muted" />
              </div>
              <p className="text-sm font-bold text-text mb-1">{t.noOrders}</p>
              <p className="text-xs text-text-muted max-w-xs">
                {isAr
                  ? 'عندما يقدّم العملاء طلبات، ستظهر هنا.'
                  : locale === 'en'
