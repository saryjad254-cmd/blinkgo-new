...
export function AdminDriversClient({
  ...
  const toggleActive = async (d: any) => {
    ...
    const willBlock = d.is_active; // currently active → next state is blocked
    const confirmMsg = willBlock
      ? (locale === 'ar'
        ? `هل تريد حقًا حظر ${d.name}؟ سيُمنع من قبول الطلبات الجديدة.`
        : locale === 'en'
        ? `Are you sure you want to block ${d.name}? They will be unable to accept new orders.`
        : `${d.name} wirklich sperren? Der Fahrer kann keine neuen Bestellungen mehr annehmen.`)
      : (locale === 'ar'
        ? `إلغاء حظر ${d.name}؟`
        : locale === 'en'
        ? `Unblock ${d.name}?`
