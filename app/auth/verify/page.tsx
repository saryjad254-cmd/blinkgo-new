...
const COPY: Record<string, any> = {
  de: {
    ...
    subtitle: 'Wir haben einen 6-stelligen Code an Ihre E-Mail gesendet',
    ...
    checkSpam: 'Prüfen Sie Ihren Spam-/Werbung-Ordner. Die E-Mail kommt von BlinkGo. Sie können auch unten auf "Code anzeigen" klicken.',
    ...
    successDesc: 'Sie werden weitergeleitet…',
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // v80 (audit open-redirect): defense-in-depth — refuse to push
        // to any path that doesn't pass isSafeRedirectUrl. The target is
        // constructed client-side from a (server-validated) email value,
        // so a malformed email could in principle inject a query string
        // that confuses a downstream redirect.
        router.push(isSafeRedirectUrl(target) ? target : '/login?verified=1');
      }, 1500);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-ink-900 relative overflow-hidden" dir={dir}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full bg-brand-yellow-500/15 blur-[100px]" />
      </div>
            ...
            <div className="text-center py-8">
              ...
              <button
                onClick={() => {
                  const params = new URLSearchParams({ email, verified: '1' });
                  const target = `/login?${params.toString()}`;
                  // v80 (audit open-redirect): defense-in-depth — refuse
                  // to navigate off-origin even if `email` is malformed.
                  router.push(isSafeRedirectUrl(target) ? target : '/login?verified=1');
                }}
