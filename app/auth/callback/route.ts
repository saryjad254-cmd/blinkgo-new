...
function getServiceClient() {
  // NOTE: the identifier `createServiceClient` previously aliased the RAW
  // supabase-js createClient. With the deployed `sb_secret_*` key that sends
  // `Authorization: Bearer sb_secret_…`, which PostgREST rejects (PGRST301),
  // so every profile read/create in this callback silently failed. This now
  // uses the hardened helper. OAuth logic itself is unchanged.
