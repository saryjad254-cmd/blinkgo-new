...
export default function CartPage() {
  ...
  // v83 fix (SENIOR-QA-V83-4): cross-tab key sync. A tab id so we can
  // ignore our own broadcast messages, plus a BroadcastChannel ref we
  // keep open during submit so other tabs can learn the current key.
  const cartTabIdRef = useRef<string>(crypto.randomUUID());
  const cartBroadcastChannelRef = useRef<BroadcastChannel | null>(null);
  ...
  async function submitOrder() {
    ...
    // Client-side delivery zone check (immediate feedback before API call).
    // Canonical: validates the customer address against the BlinkGo service
    // center (Wesseling) + 15 km radius — NOT a restaurant-specific radius.
    // Matches the server-side check in /api/orders.
    ...
    // v83 fix (SENIOR-QA-V83-4): cross-tab idempotency key broadcast.
    // The previous implementation used a useRef that was per-tab, so
    // opening the cart in two tabs would produce two UUIDs and two
    // orders. BroadcastChannel lets tabs share a single in-flight
    // idempotency key. If another tab has already submitted (or is
    // submitting), the broadcast will set our ref to the same key
    // before the request fires, ensuring the server sees the same
    // (key, scope) tuple and dedupes the second request.
    try {
      const ch = new BroadcastChannel('blinkgo-cart-idempotency');
      ch.postMessage({ type: 'request-key', tabId: cartTabIdRef.current });
      ch.onmessage = (e) => {
        const msg = e.data;
        if (msg?.type === 'current-key' && msg?.key) {
          if (idempotencyKeyRef.current !== msg.key) {
            idempotencyKeyRef.current = msg.key;
          }
        } else if (msg?.type === 'request-key') {
          if (idempotencyKeyRef.current) {
            ch.postMessage({ type: 'current-key', key: idempotencyKeyRef.current });
          }
        }
      };
      // Hold the channel open until the submit completes (closed in
      // the finally block below). We can't close immediately because
      // sibling tabs need time to reply.
      cartBroadcastChannelRef.current = ch;
    } catch {
      // BroadcastChannel not supported (very old browsers / SSR)
    }
    ...
    try {
      ...
      // v82 fix: do NOT clear the idempotency key on AbortError. The
      // server may have received the request and just not responded in
      // time; clearing the key would let a retry create a duplicate
      // order. The server-side idempotency cache (in-memory +
      // idempotency_keys DB table) will dedupe on the next retry.
      if (e?.name === 'AbortError') {
        setError(
          t.errors?.timeout ??
            'Die Verbindung dauerte zu lange. Bitte prüfe dein Internet und versuche es erneut.',
        );
      } else {
      ...
      setSubmitting(false);
      // Close the broadcast channel — any sibling tabs that wanted the
      // key have already had time to grab it.
      if (cartBroadcastChannelRef.current) {
        try { cartBroadcastChannelRef.current.close(); } catch { /* ignore */ }
        cartBroadcastChannelRef.current = null;
      }
    }
  }
          ...
          <div className="divide-y divide-edge">
            {items.map((item) => (
              <div key={item.product_id} className="flex items-center gap-3 p-4">
                {item.image_url ? (
                  // v82 perf: next/image with width/height reserves space
                  // (no CLS) and serves AVIF/WebP via /_next/image.
                  <Image
                    src={item.image_url}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-2">
                    <button
                      ...
                      aria-label={locale === 'ar' ? 'إنقاص الكمية' : locale === 'en' ? 'Decrease quantity' : 'Menge verringern'}
                      className="w-7 h-7 rounded-lg bg-surface border border-edge hover:border-brand-red-500 hover:bg-brand-red-500/10 text-white flex items-center justify-center transition-all focus:outline-none focus:ring-2 focus:ring-brand-red-500/50"
                    >
                    ...
                    <button
                      ...
                      aria-label={locale === 'ar' ? 'زيادة الكمية' : locale === 'en' ? 'Increase quantity' : 'Menge erhöhen'}
                      className="w-7 h-7 rounded-lg bg-surface border border-edge hover:border-brand-red-500 hover:bg-brand-red-500/10 text-white flex items-center justify-center transition-all focus:outline-none focus:ring-2 focus:ring-brand-red-500/50"
                    >
        ...
        >
          {submitting ? (
            <>
              ...
              <span>
                {locale === 'ar' ? 'جاري إرسال الطلب...' : locale === 'en' ? 'Placing order...' : 'Wird bestellt...'}
              </span>
            </>
          ) : (
            <>
        ...
        <p className="text-center text-xs text-text-muted">
          {(t as any).customer?.termsNotice ?? (
            locale === 'ar'
              ...
              : 'Mit dem Tippen auf „Bestellung aufgeben" bestätigen Sie unsere AGB und den angezeigten Gesamtbetrag.'
          )}
        </p>
      </div>
    </>
