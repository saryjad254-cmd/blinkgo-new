...
export default async function FavoritesPage() {
  ...
  const { data: favs, error } = await supabase
    ...
    .select(`
      ...
      restaurants:restaurant_id (
        ...
        delivery_fee, estimated_delivery_time, cover_url, cuisine, is_active, is_featured
      )
    `)
    .eq('user_id', user.id)
  ...
  if (error && !tableMissing) {
    logger.error('favorites.page: failed to load favorites', { userId: user.id, error: error.message, code: error.code });
    return (
      <>
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
          <EmptyState
            ...
            description={
              locale === 'ar'
                ? 'تعذّر تحميل المفضّلة. يرجى المحاولة مرة أخرى.'
                : locale === 'en'
