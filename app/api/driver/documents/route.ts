...
export async function GET(): Promise<NextResponse> {
  return (await withSecurity(
    ...
    async () => listDocuments() as any,
  )({} as NextRequest)) as unknown as NextResponse;
}

async function listDocuments(): Promise<NextResponse> {
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  return (await withSecurity(
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async (_ctx, r) => uploadDocument(r as NextRequest) as any,
  )(req)) as unknown as NextResponse;
}

async function uploadDocument(req: NextRequest): Promise<NextResponse> {
  return withErrorHandling(async () => {
    const supabaseAuth = createServerClient();
    const { data: { user } } = await supabaseAuth.auth.getUser();
    if (!user) throw new AuthenticationError();
