...
export async function GET(req: NextRequest): Promise<NextResponse> {
  return withErrorHandling(async () => {
    ...
    // so a future code change that ever lets user input reach this string
    // can NEVER redirect off-origin.
    if (!isSafeRedirectUrl(dashboard)) {
      logger.error('magic_link_verify dashboard path failed safety check', {
        userId: user.id,
        role: user.role,
        dashboard,
      });
