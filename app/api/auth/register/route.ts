...
/**
 * v80 (audit F-03): shape-and-message-equal success response.
 * Returned in BOTH the "new email" path and the "email already registered"
 * path so an attacker cannot enumerate registered emails by observing
 * the API response. The same 200/ok envelope is used; the verification
 * email is only sent on the new-email path.
 */
const ENUM_SAFE_OK_BODY = {
  ok: true as const,
  message:
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  return withErrorHandling(async () => {
    ...
    if (existingProfile) {
      ...
      // v80 (audit F-03): do NOT throw 409 here — that would let an attacker
      // enumerate registered emails. Return the same 200/ok envelope as the
      // new-email path. The verification email is intentionally NOT sent
      // (no need to re-verify an already-verified address).
    ...
    //    - If verified → real duplicate → 200/ok (no enumeration) [v80 F-03]
    //    - If not verified → stale partial signup → delete and recreate
    if (existingAuthUser) {
      if (existingAuthUser.email_confirmed_at) {
        // v80 (audit F-03): same shape as the new-email success response
        return ok({ ...ENUM_SAFE_OK_BODY });
      }
    ...
    if (profileErr) {
      ...
      if (profileErr.code === '23505') {
        // v80 (audit F-03): race with another tab — same 200/ok envelope.
        return ok({ ...ENUM_SAFE_OK_BODY });
      }
