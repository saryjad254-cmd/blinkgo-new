...
export async function POST(req: NextRequest): Promise<NextResponse> {
  return (await withSecurity(
    // Logout is a sensitive state change — strict rate limit; auth not required
    // (the user might have an expired session but still want to clear cookies).
    secureRoute('auth'),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async (_ctx, r) => logoutHandler(r as NextRequest) as any,
