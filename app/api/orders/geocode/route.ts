...
export async function POST(req: NextRequest) {
  return (await withSecurity(
    secureRoute('moderate', ['customer', 'driver', 'admin', 'super_admin', 'manager']),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async (_ctx, r) => geocodeOriginal(r as NextRequest) as any,
...
async function geocodeOriginal(req: NextRequest) {
  ...
  if (batch) {
    // v81 (BLOCKER fix per backend audit #8): read the role from
    // public.users (authoritative) — never from user.user_metadata,
    // which a compromised JWT or self-service endpoint could mutate.
    const { data: callerProfile } = await service
      .from('users')
      .select('role')
      .eq('id', user.id)
      .single();
    const role = callerProfile?.role ?? 'customer';
