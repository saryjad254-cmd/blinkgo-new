...
export async function GET(req: NextRequest) {
  ...
  if (type === 'all' || type === 'restaurant') {
    const rPromise = (async () => {
      try {
        let r = supabase
          .from('restaurants')
          .select('id, name, cover_url, rating, review_count, cuisine, delivery_fee, estimated_delivery_time, address, latitude, longitude, min_order_amount')
          .limit(limit)
        ...
        if (query) {
          try {
            ...
            const safeQ = escapeIlike(query);
            r = r.or(`name.ilike.%${safeQ}%,description.ilike.%${safeQ}%`);
          } catch {}
        }
  ...
  if (type === 'all' || type === 'product') {
    const prPromise = (async () => {
      ...
      // column.
      const FULL_SELECT =
        'id, name, description, price, image_urls, restaurant_id, is_active, sold_count, restaurants:restaurant_id(id, name, is_active, delivery_fee)';
      const MIN_SELECT =
        'id, name, description, price, image_urls, restaurant_id, restaurants:restaurant_id(id, name, is_active, delivery_fee)';
      ...
      async function runQuery(selectClause: string, withIsActiveFilter: boolean) {
        ...
        if (withIsActiveFilter) pr = pr.eq('is_active', true);

        if (query) {
          // v80 BLOCKER fix: escape user input (PostgREST filter injection)
          const { escapeIlike } = await import('@/lib/api/escape-ilike');
          const safeQ = escapeIlike(query);
          pr = pr.or(`name.ilike.%${safeQ}%,description.ilike.%${safeQ}%,category.ilike.%${safeQ}%`);
        }
      ...
      try {
        return await runQuery(FULL_SELECT, true);
      } catch (e: any) {
        // Defensive fallback: missing column on the production DB.
        // Re-run with the minimal select and without the is_active /
        // sold_count filters so the search still works.
        if (
          /column .* does not exist|is_active|sold_count|image_urls/i.test(
            e?.message || ''
          )
        ) {
          logger.warn('Search products: column missing, using minimal select', {
            error: e?.message,
          });
          return await runQuery(MIN_SELECT, false);
        }
        throw e;
      }
    })();
