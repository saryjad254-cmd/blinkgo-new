...
export async function GET(req: NextRequest) {
  ...
  if (type === 'all' || type === 'product') {
    const prPromise = (async () => {
      let pr = supabase
        .from('products')
        .select('id, name, description, price, image_urls, restaurant_id, is_active, sold_count, restaurants:restaurant_id(id, name, is_active, delivery_fee)')
        .eq('is_active', true)
        .limit(limit)
        .range(offset, offset + limit - 1);
  ...
  try {
    ...
    // Normalize the cover field in code (the query no longer aliases a
    // specific column, so it works whichever of cover_url/image_url exists).
    let restaurants = (restaurantsRes.data || []).map((r: any) => ({
      ...r, cover_url: restaurantCoverUrl(r),
    })).filter((r: any) => {
